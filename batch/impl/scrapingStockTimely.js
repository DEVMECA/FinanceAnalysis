    ...
    getHTML(obj)
      .then((html) => {
        ...
        $stockList.each(function (i, el) {
          ...
          const stk_date = obj.stk_date;
          const stk_time = $(this).children('td').eq(0).text()
                              .replace(/\n/g,'').replace(/\t/g,'');
          const chc_amt = $(this).children('td').eq(1).text().replace(/,/g,'');
          
          const stk_cpr_bef_amt = $(this).children('td').eq(2).text().replace(/,/g,'')
                                  .replace(/\n/g,'').replace(/\t/g,'');
          const sel_amt = $(this).children('td').eq(3).text().replace(/,/g,'');
          const buy_amt = $(this).children('td').eq(4).text().replace(/,/g,'');
          const trd_amt = $(this).children('td').eq(5).text().replace(/,/g,'');
          const chg_amt = $(this).children('td').eq(6).text().replace(/,/g,'');

          if(stk_time.length<4)
            return true;
...
module.exports = ScrapingStockTimely